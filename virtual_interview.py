...
st.set_page_config(page_title="가상 면접 챗봇")
st.title("포트폴리오 기반 가상 면접 질문 챗봇")
...
submit_button = st.button("질문 생성")
# 13. 사용자 질의 입력 및 답변 출력

if submit_button:
    with st.spinner("질문 생성 중..."):
        # RAG 파이프라인 실행: {"question": 질문} 형태로 입력
        result = st.session_state.rag_chain.invoke({"question": "주어진 참고 문서를 참고해 기술적인 면접 질문을 해줘"})
        st.write("### 질문 : ")
